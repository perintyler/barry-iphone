...
enum ProbeOutcome: Equatable {
    ...
    /// Whether the app can actually load the sessions list with this
    /// configuration.
    ...
    var message: String {
        ...
        case .working(let count):
            return "Connected and authorized — the server returned \(count) session\(count == 1 ? "" : "s")."
        case .reachableButUnauthorized:
...
/// from "the secret is wrong"; the sessions route then proves the credential
...
struct ConnectionProbe {
    ...
    func run() async -> ProbeOutcome {
        ...
        do {
            let page = try await BarryClient(config: config, urlSession: urlSession).sessions(limit: 1)
            return .working(sessionCount: page.sessions.count)
        } catch let error as BarryError {
    ...
    static func classify(apiError error: BarryError) -> ProbeOutcome {
        switch error {
        case .badURL:
            return .badURL
