...
final class ConnectionProbeTests: XCTestCase {
    ...
    func testForbiddenIsReachableAndSaysTheSecretIsTheProblem() {
        ...
        XCTAssertFalse(outcome.isFullyWorking, "but the app still cannot list sessions")
    ...
    ///
    /// The health route answers 502 while the sessions route answers 403. Those
    /// MUST differ: a stub returning one status to both would leave this test
    /// green even with the health check deleted, because the sessions call
    ...
    func testAnUnhealthyServerIsNotBlamedOnTheSecret() async {
        let outcome = await ConnectionProbe(
            ...
            urlSession: StubURLProtocol.session(health: 502, sessions: 403)
        ).run()

        guard case .serverError(let status, _) = outcome else {
            return XCTFail("a 502 from the health route should outrank the sessions route's 403, got \(outcome)")
        }
        XCTAssertEqual(status, 502)
        XCTAssertFalse(outcome.message.lowercased().contains("secret"),
    ...
    func testAHealthyServerThatRefusesTheSecretReportsUnauthorized() async {
        let outcome = await ConnectionProbe(
            ...
            urlSession: StubURLProtocol.session(health: 200, sessions: 403)
        ).run()
    ...
    /// The real API with no secret. This is exactly what the user saw from
    /// their phone, and it must come back as "reachable, fix the secret".
    ///
    /// Skips only when the server is genuinely absent — never when it answers,
    /// because an answer is the thing under test.
    func testRealAPIWithNoSecretReportsUnauthorized() async throws {
    ...
    func testRealProxyReportsFullyWorkingWithSessions() async throws {
        ...
        XCTAssertGreaterThan(count, 0, "the machine has sessions; a limit=1 page should return one")
    }
...
/// Answers the health route and the sessions route with SEPARATE canned
...
private final class StubURLProtocol: URLProtocol {
    ...
    nonisolated(unsafe) static var sessionsStatus = 200

    static func session(health: Int, sessions: Int) -> URLSession {
        healthStatus = health
    ...
    override func startLoading() {
        ...
        let status = isHealth ? Self.healthStatus : Self.sessionsStatus
        let response = HTTPURLResponse(
            url: request.url!, statusCode: status, httpVersion: nil, headerFields: nil
        )!
        client?.urlProtocol(self, didReceive: response, cacheStoragePolicy: .notAllowed)
        client?.urlProtocol(self, didLoad: Data(#"{"ok":true,"sessions":[]}"#.utf8))
